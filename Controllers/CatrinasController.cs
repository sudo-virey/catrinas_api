...
public class CatrinasController : ControllerBase
{
    ...
    // Método para actualizar la tabla Rankings con totales por categoría
    private async Task ActualizarRankingConTotales()
    {
        // Obtener datos calculados de evaluaciones activas por participante calificado
        var datosRanking = await _context.Participantes
            .Where(p => p.Id_Estado == 3) // Solo participantes calificados
            .GroupJoin(_context.Evaluaciones.Where(e => e.Activo),
                p => p.Id_Participante,
                e => e.Id_Participante,
        ...
        foreach (var datos in datosRanking)
        {
            // Buscar si ya existe un registro en Rankings para este participante
            var rankingExistente = await _context.Rankings
                .FirstOrDefaultAsync(r => r.Id_Participante == datos.IdParticipante);

            if (rankingExistente != null)
            {
                // Actualizar registro existente
                rankingExistente.Puntos = datos.SumaTotal;
                rankingExistente.TotalAtuendo = datos.SumaAtuendo;
                rankingExistente.TotalMaquillaje = datos.SumaMaquillaje;
                rankingExistente.TotalTradiciones = datos.SumaTradiciones;
            ...
            else
            {
                ...
                var nuevoRanking = new Ranking
                {
                    Id_Participante = datos.IdParticipante,
                    Puntos = datos.SumaTotal,
                    TotalAtuendo = datos.SumaAtuendo,
                    TotalMaquillaje = datos.SumaMaquillaje,
                    TotalTradiciones = datos.SumaTradiciones,
                    TotalPasarela = datos.SumaPasarela,
                    TotalInteraccion = datos.SumaInteraccion,
